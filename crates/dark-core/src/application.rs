...
impl ArkService {
    ...
    async fn asp_create_and_submit_partial_sigs(
        ...
        for node in &tree_nodes {
            ...
            // Build musig2 pubkeys preserving original parity (02/03 prefix) from
            // the PSBT. The Go SDK's btcd musig2 does full-point equality checks,
            // so we must use the exact same keys (including parity) for signing.
            // The keys are sorted for deterministic aggregation (BIP-327).
    ...
    async fn aggregate_tree_signatures(&self, batch_id: &str) -> ArkResult<()> {
        ...
        for node in &round.vtxo_tree {
            ...
            // Build KeyAggContext preserving original parity (02/03 prefix) from
            // the PSBT. The Go SDK's btcd musig2 does full-point equality checks,
            // so we must use the exact same keys (including parity) for signing.
            // The keys are sorted for deterministic aggregation (BIP-327).
            let mut musig_pubkeys: Vec<musig2::secp256k1::PublicKey> = Vec::new();
